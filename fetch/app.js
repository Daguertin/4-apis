...
//json is a method of a response object and returns a promise.
fetch(url)
.then(res => res.json())
.then(data => {
        console.log(data)
...
async function getData() { //getting the specific profile and abilities of pika or a single body class
    try {
        const res = await fetch(`${url}pikachu`)
        const data = await res.json()//provides a response over a promise
        console.log(data)
    } catch(err) {
        console.log(err)
    }
...
// getData()
    ...
    // think of .then, .catch, .finally and compare them to how if else runs
    // ! Using try catch process will help with using an async function
    // Do not forget to append data
    // imagine a pokemon game where it teaches you to code. (catch erros handling etc.)